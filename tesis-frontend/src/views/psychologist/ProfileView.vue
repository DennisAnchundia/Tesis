              ...
              <div class="mt-4 sm:mt-0 flex-1 min-w-0 text-center sm:text-left">
                ...
                <div class="flex justify-center sm:justify-start items-center space-x-4 sm:space-x-6 mt-3 sm:mt-4">
                  <div class="text-center">
                    ...
                    <div class="text-xs sm:text-sm text-gray-500">Estudiantes</div>
                  </div>
  ...
  methods: {
    ...
    async fetchUser() {
      try {
        ...
        // Si el backend devuelve el user directamente
        const user = res.data.user || res.data; // según cómo venga
        this.user = user;
...
async uploadImage(type, file) {
  try {
    ...
    const { data } = await axios.put(
      `http://localhost:3000/api/users/upload/${this.user._id || this.user.uid}`,
      formData,
      {
        headers: {
          'x-token': token,
...
if (data.user.profileImage) {
  this.profileImageUrl = '';  // borrar primero
  this.$nextTick(() => {
    this.profileImageUrl = `http://localhost:3000/${data.user.profileImage}?t=${timestamp}`;
  });
}
if (data.user.coverImage) {
  this.coverImageUrl = '';
  this.$nextTick(() => {
    this.coverImageUrl = `http://localhost:3000/${data.user.coverImage}?t=${timestamp}`;
  });
}
    this.user = data.user;
  } catch (err) {
